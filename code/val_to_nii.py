...
def get_args_parser():
    parser = argparse.ArgumentParser('Transform CBCT and Implant data into validation set', add_help=False)
    ...
    parser.add_argument('--images_path', type=str, default='./data/nii/val/raw')
    parser.add_argument('--labels_path', type=str, default='./data/nii/val/label')
    parser.add_argument('--train_data_size', type=int, default=1220)
...
def val_to_nii(args) :
    ...
    for it in range(len(dicom_dirs)) :
        ...
        for jt in range(args.augment_size) :
            ...
            image_number = str(it * args.augment_size + jt + args.train_data_size + 1).zfill(4)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser('Transform CBCT and Implant data into validation set', parents=[get_args_parser()])
