...
class Encoder(nn.Module):
    def __init__(self,
        ...
        self.feature_embed = nn.Conv3d(in_channels=inp_channels, out_channels=dim, kernel_size=3, padding=1, stride=1,
                                       groups=1, bias=True)
        self.b1 = nn.Sequential(*[NAFBlock(dim) for _ in range(num_blocks[0])])
        self.down1 = nn.Conv3d(dim, 2 * dim, 2, 2)
        self.b2 = nn.Sequential(*[NAFBlock(dim * 2) for _ in range(num_blocks[1])])
        self.down2 = nn.Conv3d(dim * 2, dim * 2 ** 2, 2, 2)
        self.b3 = nn.Sequential(*[NAFBlock(dim * 2 ** 2) for _ in range(num_blocks[2])])
        self.down3 = nn.Conv3d(dim * 2 ** 2, dim * 2 ** 3, 2, 2)
...
class Deblur_head(nn.Module):
    def __init__(self, num_in, num_mid, num_out):
        ...
        self.block = nn.Sequential(
            ...
            # nn.GELU(),
            nn.Conv3d(num_in, num_out, kernel_size=3, stride=1, padding=1),
...
class Decoder(nn.Module):
    def __init__(self,
        ...
        self.up21 = nn.Sequential(
            nn.Conv3d(dim * 2 ** 1, dim * 2 ** 2, 1, bias=False),
            nn.PixelShuffle(2)
        )
