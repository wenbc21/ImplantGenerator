...
def make_dataset(args) :
    ...
    for it in range(len(cbct_data_path)) :
        ...
        dicom_pad = np.pad(dicom, pad_width=((48, 48), (48, 48), (48, 48)), mode='constant', constant_values=0)
        dicom_part = dicom_pad[
            int(midx - half_patch + 48): int(midx + half_patch + 48), 
            int(midy - half_patch + 48): int(midy + half_patch + 48), 
            int(midz - half_patch + 48): int(midz + half_patch + 48), 
        ]
        ...
        if is_test:
            ...
            sitk.WriteImage(dicom_img, os.path.join(args.results_path, f"imagesTs", f"IMPLANT_{dataset_name}_{data_name}_0000.nii.gz"))
            sitk.WriteImage(implant_img, os.path.join(args.results_path, f"labelsTs", f"IMPLANT_{dataset_name}_{data_name}.nii.gz"))
